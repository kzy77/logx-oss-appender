...
- [x] Task 7: 实现SF OSS适配器 (AC: 6-10)
  - [x] 创建SfOssStorageAdapter类，实现统一存储服务接口
  - [x] 集成SF OSS SDK，配置region和credentials
  - [x] 实现putObject方法，支持分片上传
...
- 配置类: `log-java-producer/src/main/java/org/logx/storage/s3/`
...
- src/main/java/org/logx/storage/StorageConfig.java - 通用存储配置管理
- src/main/java/org/logx/storage/s3/S3StorageAdapter.java - S3存储适配器
