...
- [ ] Task 1: 创建S3StorageAdapter核心类 (AC: 1)
  - [ ] 在log-java-producer模块创建S3StorageAdapter类
  - [ ] 继承S3StorageInterface接口（来自故事1.2）
  - [ ] 实现StorageBackend.AWS_S3枚举支持
  - [ ] 添加类级别Javadoc说明AWS S3集成功能
  - [ ] 确保与OSSStorageAdapter接口一致性

- [ ] Task 2: 集成AWS SDK v2配置 (AC: 2)
  - [ ] 确认AWS SDK v2 2.28.16依赖已在父POM中配置
  - [ ] 创建S3StorageConfig配置类继承基础S3StorageConfig
  - [ ] 配置AWS特定参数：region、accessKeyId、secretAccessKey
  - [ ] 实现S3Client客户端初始化，支持区域自动检测
  - [ ] 添加连接池和超时配置优化

- [ ] Task 3: 实现putObject和multipart upload (AC: 3)
  - [ ] 实现putObject(String key, byte[] data)标准上传
  - [ ] 实现multipart upload支持大文件分片上传
  - [ ] 添加自动分片逻辑：>5MB数据使用multipart
  - [ ] 实现putObjects批量上传优化
  - [ ] 支持S3对象元数据设置（Content-Type、Content-Encoding）

- [ ] Task 4: 实现AWS特定错误处理和重试 (AC: 4)
  - [ ] 实现S3Exception特定异常处理逻辑
  - [ ] 集成AWS SDK内置重试策略与自定义重试结合
  - [ ] 添加AWS服务错误分类：ThrottlingException、InternalError等
  - [ ] 实现S3服务限流和配额错误的特殊处理
  - [ ] 添加AWS错误码和RequestId的详细日志

- [ ] Task 5: 实现S3健康检查和权限验证 (AC: 1, 2)
  - [ ] 实现healthCheck()方法验证S3连接和权限
  - [ ] 添加bucket存在性和访问权限检查
  - [ ] 实现区域配置验证和自动纠错
  - [ ] 添加S3服务可用性检测
  - [ ] 提供S3连接诊断和故障排除信息

- [ ] Task 6: 编写S3特定单元测试 (AC: 5)
  - [ ] 创建S3StorageAdapterTest测试类
  - [ ] 测试标准上传和multipart upload流程
  - [ ] 模拟AWS S3错误和网络异常情况
  - [ ] 测试重试逻辑和AWS特定错误处理
  - [ ] 验证配置参数和区域设置正确性
  - [ ] 达到>90%代码覆盖率要求
...
**平行开发**: 与故事1.3（阿里云OSS适配器）保持接口一致性

**为后续故事准备**: 为故事1.6健康检查功能提供S3连接验证能力
...
**接口一致性**: 与OSSStorageAdapter保持相同的方法签名和行为模式
