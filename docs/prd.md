...
### Story 1.2：设计统一存储服务接口
**As a** 开发者，
**I want** 定义基于存储服务的统一接口，
**so that** 可以支持多种云存储后端的无缝切换。

#### 验收标准
1. 定义StorageService接口，包含putObject()和putObjects()核心方法
2. 设计配置抽象类，支持endpoint、credentials、bucket等参数
3. 实现错误处理和重试策略的接口定义
4. 创建存储后端枚举（OSS、S3、MinIO、SF OSS等）
5. 编写接口文档和使用示例
...
### Story 1.5：实现SF OSS适配器
**As a** 使用SF OSS的开发者，
**I want** 有SF OSS的具体实现适配器，
**so that** 可以将日志上传到SF对象存储。

#### 验收标准
1. 实现SfOssStorageAdapter类，实现统一存储服务接口
2. 集成SF OSS SDK，配置region和credentials
3. 实现putObject方法，支持分片上传
4. 添加SF OSS特定的错误处理和重试逻辑
5. 编写SF OSS连接和上传的单元测试
...
**As a** 使用S3兼容存储的开发者，
**I want** 有独立的S3适配器模块，
**so that** 可以按需引入S3相关的依赖。

#### 验收标准
1. 创建logx-s3-adapter模块，包含所有S3相关的依赖
2. 移动现有的S3相关代码到此模块
3. 实现StorageService接口的S3存储服务
4. 配置Java SPI服务发现文件
5. 编写模块使用文档和示例

### Story 5.3：实现SF OSS适配器模块
**As a** 使用SF OSS的开发者，
**I want** 有独立的SF OSS适配器模块，
**so that** 可以按需引入SF OSS相关的依赖。
...
### Story 5.4：更新框架适配器以支持新架构
**As a** 框架适配器用户，
**I want** 框架适配器支持新的模块化架构，
**so that** 可以继续使用现有的配置方式。

#### 验收标准
1. 更新所有框架适配器以使用新的存储服务工厂
2. 保持向后兼容性，用户无需修改现有配置
3. 添加可选的backendType配置参数
4. 编写迁移指南和兼容性测试
5. 验证各种组合的集成场景
